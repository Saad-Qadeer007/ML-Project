...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.caption("Set the local energy unit price to calculate estimated costs.")
    ...
    if st.button("⚡ Run Predictive Analysis", type="primary", use_container_width=True):
        with st.spinner("Analyzing data..."):
            ...
            # 3. Integration Formula
            final_heating = base_heating * occupancy_pct
            final_cooling = base_cooling * occupancy_pct

            # 4. Cost Calculation
            base_total_energy = base_heating + base_cooling
            final_total_energy = final_heating + final_cooling
            base_cost = base_total_energy * unit_price
            final_cost = final_total_energy * unit_price
        ...
        c4.metric("Total Guests × Stay Factor", f"{guests * stay_length} units")
        ...
        st.markdown("<hr class='custom-divider'>", unsafe_allow_html=True)
        st.markdown("### 🧮 Integration Formula")
        st.latex(r"\text{Final Energy} = \text{Base Energy} \times \text{Occupancy\%}")
        st.info(f"Occupancy **{occupancy_pct*100:.1f}%** → Heating reduced from **{base_heating:.2f}** to **{final_heating:.2f} kWh/m²** | Cooling reduced from **{base_cooling:.2f}** to **{final_cooling:.2f} kWh/m²**")

        st.markdown("<hr class='custom-divider'>", unsafe_allow_html=True)
        st.markdown("### 💰 Financial Impact")
        f1, f2, f3 = st.columns(3)
        f1.metric("Base Energy Cost", f"${base_cost:.2f}")
        f2.metric("Optimized Energy Cost", f"${final_cost:.2f}", delta=f"-${savings:.2f}", delta_color="normal")
